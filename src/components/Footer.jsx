...
const LinkBox = ({ title, links }) => {
  ...
  return (
    <div>
      ...
      <ul className="text-gray-500 dark:text-gray-400">{link}</ul>
    </div>
  );
};
...
const Footer = () => {
  const linkeBox = footerLinks.map((link, index) => (
    <LinkBox key={index} title={link.title} links={link.links} />
  ));

  const iconContainer = icons.map((icn, index) => (
    <li key={index}>
      <a
        href="#"
